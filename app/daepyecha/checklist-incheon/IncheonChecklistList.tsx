...
// ─────────────────────────────────────────────────────────────
// 인천 B820 설치완료 체크리스트 관리 목록
//   검색(운수사/차량) + 기간(설치일) + 선택 ZIP + 수정 + 휴지통
//   백엔드는 기존 /api/checklist 공유(variant=incheon 로 분리). 센터 "인천" 고정.
// ─────────────────────────────────────────────────────────────
import { useCallback, useEffect, useRef, useState } from "react";
import type { IncRow } from "@/lib/checklist-incheon/types";
import IncheonChecklistModal from "./IncheonChecklistModal";

type ListRow = Omit<IncRow, "data">;

export default function IncheonChecklistList() {
  ...
  async function downloadSelected() {
    ...
    try {
      ...
      a.href = url;
      a.download = "checklist-incheon.zip";
  ...
  return (
    <div className="flex flex-col gap-3">
      <header className="flex flex-col items-center pt-1 text-center">
        <h1 className="text-lg font-extrabold text-slate-800">인천 설치완료 체크리스트</h1>
        <p className="mt-1 text-xs text-slate-400">인천 B820 기종. 점검·서명 후 PDF로 보관됩니다.</p>
      </header>
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          {trashed && rows.length > 0 && (
            <button onClick={() => act("/api/checklist/trash?variant=incheon", "DELETE", "휴지통을 비우면 모두 영구 삭제됩니다. 진행할까요?")} disabled={busy} className="rounded-lg border border-red-200 px-2 py-1.5 text-xs font-bold text-red-600">
              비우기
            </button>
          )}
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {r.vehicle_numbers ? `${r.vehicle_numbers} · ` : ""}설치일 {r.install_date || "-"}
                  </p>
                  <p className="mt-0.5 text-xs text-slate-400">
                    설치자 {r.installer_name || "-"}
      ...
      {modal && (
        <IncheonChecklistModal editId={modal === "new" ? undefined : modal} onClose={() => setModal(null)} onSaved={() => { setModal(null); load(); }} />
      )}
    </div>
  );
}
