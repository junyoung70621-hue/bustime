  ...
  return (
    <div className="flex flex-col gap-3">
      <header className="flex flex-col items-center pt-1 text-center">
        <h1 className="text-lg font-extrabold text-slate-800">설치 완료 체크리스트</h1>
        <p className="mt-1 text-xs text-slate-400">점검·서명 후 PDF로 보관됩니다.</p>
      </header>
      ...
      <div className="flex gap-1.5">
        {(["", ...CENTERS] as const).map((c) => (
          <button key={c || "전체"} onClick={() => setFilter(c)} className={`h-9 flex-1 rounded-lg text-sm font-semibold transition ${filter === c ? "bg-brand-50 text-brand-700 ring-1 ring-brand-200" : "bg-white text-slate-500 ring-1 ring-slate-200 hover:bg-slate-50"}`}>
            {c || "전체"}
          </button>
        ))}
      </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <p className="flex items-center gap-1.5">
                    <span className="rounded-md bg-slate-100 px-2 py-0.5 text-xs font-bold text-slate-600">{r.center}센터</span>
      ...
      {modal && (
        <ChecklistModal editId={modal === "new" ? undefined : modal} onClose={() => setModal(null)} onSaved={() => { setModal(null); load(); }} />
      )}
    </div>
  );
}
