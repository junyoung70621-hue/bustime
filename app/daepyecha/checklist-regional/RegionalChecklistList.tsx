...
// ─────────────────────────────────────────────────────────────
// 설치 완료 체크리스트(지역: 대전·세종) 관리 목록
//   검색(운수사/차량) + 기간(점검일) + 지역 필터 + 선택 ZIP + 수정 + 휴지통
//   백엔드는 기존 /api/checklist 공유(variant=regional 로 분리).
// ─────────────────────────────────────────────────────────────
import { useCallback, useEffect, useRef, useState } from "react";
import { REGIONS } from "@/lib/checklist-regional/templates";
import type { RegRow, Region } from "@/lib/checklist-regional/types";
import RegionalChecklistModal from "./RegionalChecklistModal";

type ListRow = Omit<RegRow, "data">;

export default function RegionalChecklistList() {
  ...
  async function downloadSelected() {
    ...
    try {
      ...
      a.href = url;
      a.download = "checklist-regional.zip";
  ...
  return (
    <div className="flex flex-col gap-3">
      <header className="flex flex-col items-center pt-1 text-center">
        <h1 className="text-lg font-extrabold text-slate-800">설치 완료 체크리스트 (지역)</h1>
        <p className="mt-1 text-xs text-slate-400">대전·세종 기종(B400/B500/B600/B650). 점검·서명 후 PDF로 보관됩니다.</p>
      </header>
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          {trashed && rows.length > 0 && (
            <button onClick={() => act("/api/checklist/trash?variant=regional", "DELETE", "휴지통을 비우면 모두 영구 삭제됩니다. 진행할까요?")} disabled={busy} className="rounded-lg border border-red-200 px-2 py-1.5 text-xs font-bold text-red-600">
              비우기
            </button>
          )}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-200 bg-white px-4 py-10 text-center text-sm text-slate-400 shadow-card">
          {trashed ? "휴지통이 비어 있습니다." : "저장된 체크리스트가 없습니다."}
        </div>
      ) : (
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <p className="flex items-center gap-1.5">
                    ...
                    <span className="rounded-md bg-brand-50 px-2 py-0.5 text-xs font-bold text-brand-700">{r.model}</span>
                    <span className="truncate font-bold text-slate-800">{r.operator}</span>
                  </p>
                  <p className="mt-1 text-xs text-slate-500">
                    {r.vehicle_numbers ? `${r.vehicle_numbers} · ` : ""}점검일 {r.install_date || "-"}
                  </p>
                  <p className="mt-0.5 text-xs text-slate-400">
                    점검자 {r.installer_name || "-"}
                ...
                <div className="flex shrink-0 flex-col items-end gap-1">
                  ...
                  {trashed ? (
                    ...
                    <>
                      ...
                      <button onClick={() => act(`/api/checklist/${r.id}`, "DELETE", "이 체크리스트를 휴지통으로 이동할까요?")} disabled={busy} className="rounded-lg px-2.5 py-1 text-xs font-bold text-red-500">삭제</button>
                    </>
                  )}
                </div>
              </div>
      ...
      {modal && (
        <RegionalChecklistModal editId={modal === "new" ? undefined : modal} onClose={() => setModal(null)} onSaved={() => { setModal(null); load(); }} />
      )}
    </div>
  );
}
