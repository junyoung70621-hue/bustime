// ─────────────────────────────────────────────────────────────
// 선택 확인서 여러 건 ZIP 다운로드
//   POST /api/daepyecha/download-zip  body: { ids: string[] }
//   → 각 PDF를 스토리지에서 받아 "운수사명 날짜.pdf"로 ZIP 묶어 반환
// ─────────────────────────────────────────────────────────────
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "다운로드 비활성: SUPABASE_SERVICE_ROLE_KEY 없음" },
      { status: 503 },
    );
  }
  ...
  for (const r of rows) {
    ...
    let name = `${safe(r.operator || "확인서")} ${r.issued_date || ""}`.trim() + ".pdf";
    // 동일 파일명 충돌 방지
    let n = 2;
    while (used.has(name)) name = `${safe(r.operator || "확인서")} ${r.issued_date || ""} (${n++}).pdf`;
    used.add(name);
    zip.file(name, buf);
  }
  ...
  return new NextResponse(out, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="daepyecha_${stamp}.zip"`,
    },
  });
}
