...
import GongyongModal from "./GongyongModal";

type ListRow = Omit<GongyongRow, "data">;

export default function GongyongList() {
  ...
  return (
    <div className="flex flex-col gap-3">
      <header className="flex flex-col items-center pt-1 text-center">
        <h1 className="text-lg font-extrabold text-slate-800">설치 확인서</h1>
        <p className="mt-1 text-xs text-slate-400">다차량 1장 양식. 작성·서명 후 PDF로 보관됩니다.</p>
      </header>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <p className="flex items-center gap-1.5">
                    ...
                    <span className="rounded-md bg-brand-50 px-2 py-0.5 text-xs font-bold text-brand-700">설치확인서</span>
      ...
      {modal && (
        <GongyongModal editId={modal === "new" ? undefined : modal} onClose={() => setModal(null)} onSaved={() => { setModal(null); load(); }} />
      )}
    </div>
  );
}
