...
import { REGIONS, GONGYONG_BUS_TYPES, MAX_VEHICLES, emptyVehicle, emptyGongyongForm } from "@/lib/gongyong/templates";
import type { GongyongForm, GongyongRow, GongyongBusType, GongyongInstallType, GongyongOffice, GongyongVehicle } from "@/lib/gongyong/types";
...
export default function GongyongModal({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        setData({ ...emptyGongyongForm(todayStr()), ...r.data, operatorSig: null, installerSig: null });
      } catch (e) {
  ...
  const addVeh = () => setData((d) => (d.vehicles.length >= MAX_VEHICLES ? d : { ...d, vehicles: [...d.vehicles, emptyVehicle()] }));
  ...
  async function save() {
    ...
    try {
      ...
      const dataForDb: GongyongForm = { ...data, operatorSig: null, installerSig: null };
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 sm:items-center">
      <div className="safe-bottom flex max-h-[94dvh] w-full max-w-lg flex-col rounded-t-2xl bg-white sm:rounded-2xl">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3">
          <h2 className="text-base font-bold text-slate-800">{isEdit ? "설치확인서 수정" : "설치 확인서"}</h2>
          ...
          {loading ? (
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <L t="설치확인일자"><input type="date" value={data.installDate} onChange={(e) => patch({ installDate: e.target.value })} className={inp} /></L>
                <L t="버스 구분">
        ...
        {pad === "installer" && (
          <SignaturePad title="설치자 확인 서명" initialName={data.installerName}
...
function firstMissing(d: GongyongForm): string | null {
  ...
  if (!d.installDate) return "설치확인일자를 입력하세요.";
  ...
  if (!d.installerName || !d.installerSig) return "설치자 확인 서명을 완료하세요.";
  return null;
}
